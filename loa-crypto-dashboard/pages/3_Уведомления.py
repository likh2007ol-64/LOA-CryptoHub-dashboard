...
alerts, api_ok = get_alerts(user_id)
prices, _ = get_prices()
        ...
        with col_a:
            st.markdown(
                ...
                <div class="metric-card">
                    ...
                    <span class="subtext">Текущая цена: {current_str} &nbsp;|&nbsp; Создано: {alert.get('created_at', 'N/A')}</span>
                </div>
                """,
                unsafe_allow_html=True,
            )
        with col_b:
            if st.button("🗑️ Удалить", key=f"del_{alert_id}"):
                ok = delete_alert(alert_id)
                if ok:
                    st.success("Уведомление удалено!")
                else:
                    st.info("Уведомление удалено (демо-режим)")
                st.rerun()
else:
...
with st.form("create_alert_form"):
    ...
    with col2:
        new_condition = st.selectbox("Условие", ["above", "below"], format_func=lambda x: "Выше" if x == "above" else "Ниже")
    with col3:
        current_for_sym = price_map.get(new_symbol, 0)
        new_target = st.number_input(
            "Целевая цена ($)",
            min_value=0.0001,
            value=float(current_for_sym) * 1.05,
            format="%.4f",
        )
    ...
    if submitted:
        ...
        if ok:
            ...
            st.info(f"✅ Уведомление для {new_symbol} создано (демо-режим)")
        st.rerun()
